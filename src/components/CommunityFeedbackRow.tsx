...
export default function CommunityFeedbackRow({
  ...
  const card = (f:Feedback) => {
    ...
    return (
      <div
        key={f.id}
        className={`fb-card relative shrink-0 rounded-xl bg-neutral-900/60 ${sevBorder} border flex flex-col justify-between`} 
        style={{ width:cardW, height:cardH, fontSize:12, lineHeight:'18px', padding:'22px 22px 18px 22px'}}
        aria-label={`feedback ${f.id}`}
      >
        <div className="pr-1">
          <strong className="block font-semibold text-white mb-2 leading-snug text-[13px] break-words">{f.bold}</strong>
          <p className="text-white/80 text-[12.5px] leading-relaxed break-words clamp-5 whitespace-pre-wrap">{f.text}</p>
        </div>
        <footer className="mt-4 flex items-center gap-3 pt-4">
          <div className="w-[38px] h-[38px] rounded-full bg-neutral-700/60 flex items-center justify-center text-[13px] text-white/70 select-none">👤</div>
          <div className="flex flex-col text-[11px] leading-tight">
            <span className="text-white/85 font-medium">{f.user}</span>
  ...
  <div ref={viewportRef} className="fb-viewport relative" style={{ width:'100%', height, overflow:'hidden', position:'relative', WebkitMaskImage:'linear-gradient(90deg, rgba(0,0,0,1) 0, rgba(0,0,0,1) calc(100% - 40px), rgba(0,0,0,0) 100%)', maskImage:'linear-gradient(90deg, rgba(0,0,0,1) 0, rgba(0,0,0,1) calc(100% - 40px), rgba(0,0,0,0) 100%)' }} aria-live="polite">
  ...
  {/* Left fade removed to keep leftmost card fully visible */}
